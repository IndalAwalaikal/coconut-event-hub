    ...
    >
      <div className="container mx-auto flex items-center justify-between px-4 py-3 md:py-4">
        ...
        >
          <img
            src="/logo.png"
            alt="COCONUT CC"
            className="h-8 sm:h-10 md:h-12 w-auto object-contain transition-transform duration-300 group-hover:scale-110 brightness-0 invert"
          />
        </Link>
      ...
      <div
        className={`overflow-hidden transition-all duration-300 ease-in-out md:hidden ${
          open ? "max-h-[300px] border-t border-blue-500/20" : "max-h-0"
        }`}
      >
          ...
          {navLinks.map((link, index) => {
            ...
            return (
              <Link
                key={link.to}
                to={link.to}
                className={`group relative block overflow-hidden rounded-xl px-4 py-3 text-sm font-medium transition-all duration-300 animate-slide-up ${
              ...
              >
                ...
                />
                <span
                  className={`absolute left-2 top-1/2 h-0 w-0.5 -translate-y-1/2 rounded-full bg-gradient-to-b from-[#3b82f6] to-[#60a5fa] transition-all duration-300 ${
